...
const OPENAI_KEY = process.env.OPENAI_API_KEY || (functions.config().openai && functions.config().openai.key) || null;
if (!OPENAI_KEY) {
  console.warn('OPENAI_API_KEY not configured. Use `firebase functions:config:set openai.key="..."`.');
}
...
app.post('/v1/ai', async (req, res) => {
  ...
  const userText = prompt || `Tell me about this species: ${species}`;
  try {
